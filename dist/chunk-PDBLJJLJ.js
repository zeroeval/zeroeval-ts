...
async function discoverIntegrations() {
  ...
  try {
    await import("openai");
    const { OpenAIIntegration } = await import("./openai-Z7SOE4O5.js");
    integrations.openai = OpenAIIntegration;
  } catch (_) {
  }
  try {
    await import("langchain");
    const { LangChainIntegration } = await import("./langchain-WDJTI24W.js");
    integrations.langchain = LangChainIntegration;
  } catch (_) {
  }
  try {
    await import("langgraph");
    const { LangGraphIntegration } = await import("./langgraph-7ZVFDNZK.js");
